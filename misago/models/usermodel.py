...
from misago.utils.avatars import delete_avatar_s3
        ...
        if self.avatar_temp:
            try:
                ...
                if not av_file.isdir():
                    delete_avatar_s3(settings.MEDIA_ROOT + 'avatars/' + self.avatar_temp)
                    av_file.remove()
            except Exception:
                pass
    ...
    def delete_avatar_original(self):
        if self.avatar_original:
            try:
                ...
                if not av_file.isdir():
                    delete_avatar_s3(settings.MEDIA_ROOT + 'avatars/' + self.avatar_original)
                    av_file.remove()
            except Exception:
                pass
        ...
        if self.avatar_image:
            for size in settings.AVATAR_SIZES[1:]:
                try:
                    ...
                    if not av_file.isdir():
                        delete_avatar_s3(settings.MEDIA_ROOT + 'avatars/' + str(size) + '_' + self.avatar_image)
                        av_file.remove()
                except Exception:
                    pass
            try:
